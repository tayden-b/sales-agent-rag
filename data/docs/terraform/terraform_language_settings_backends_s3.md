...
- Terraform
- [v1.15.x (alpha)](/terraform/language/v1.15.x/backend/s3)
- [v1.13.x](/terraform/language/v1.13.x/backend/s3)
- [v1.12.x](/terraform/language/v1.12.x/backend/s3)
- [v1.11.x](/terraform/language/v1.11.x/backend/s3)
- [v1.10.x](/terraform/language/v1.10.x/backend/s3)
- [v1.9.x](/terraform/language/v1.9.x/backend/s3)
- [v1.8.x](/terraform/language/v1.8.x/backend/s3)
- [v1.7.x](/terraform/language/v1.7.x/backend/s3)
- [v1.6.x](/terraform/language/v1.6.x/backend/s3)
- [v1.5.x](/terraform/language/v1.5.x/backend/s3)
- [v1.4.x](/terraform/language/v1.4.x/backend/s3)
- [v1.3.x](/terraform/language/v1.3.x/backend/s3)
- [v1.2.x](/terraform/language/v1.2.x/backend/s3)
- [v1.1.x](/terraform/language/v1.1.x/backend/s3)
...
- [`use_lockfile`](/terraform/language/backend/s3#use_lockfile) - (Optional) Whether to use a lockfile for locking the state file. Defaults to `false`.

#### Enabling DynamoDB State Locking (Deprecated)

To enable DynamoDB state locking, use the following optional arguments:

- [`dynamodb_endpoint`](/terraform/language/backend/s3#dynamodb_endpoint) - (Optional, **Deprecated**) Custom endpoint URL for the AWS DynamoDB API. Use `endpoints.dynamodb` instead.
- [`dynamodb_table`](/terraform/language/backend/s3#dynamodb_table) - (Optional, **Deprecated**) Name of the DynamoDB Table to use for state locking and consistency. The table must have a partition key named `LockID` with a type of `String`.
...
- [`s3:ListBucket`](/terraform/language/backend/s3#s3-listbucket) on `arn:aws:s3:::mybucket`. At a minimum, this must be able to list the path where the state is stored.
- [`s3:GetObject`](/terraform/language/backend/s3#s3-getobject) on `arn:aws:s3:::mybucket/path/to/my/key`
- [`s3:PutObject`](/terraform/language/backend/s3#s3-putobject) on `arn:aws:s3:::mybucket/path/to/my/key`
...
- [`s3:ListBucket`](/terraform/language/backend/s3#s3-listbucket-1) on `arn:aws:s3:::mybucket`. At a minumum, this must be able to list the path where the `default` workspace is stored as well as the other workspaces.
- [`s3:GetObject`](/terraform/language/backend/s3#s3-getobject-1) on `arn:aws:s3:::mybucket/path/to/my/key`, `arn:aws:s3:::mybucket/<workspace_key_prefix>/*/path/to/my/key`
- [`s3:PutObject`](/terraform/language/backend/s3#s3-putobject-1) on `arn:aws:s3:::mybucket/path/to/my/key`, `arn:aws:s3:::mybucket/<workspace_key_prefix>/*/path/to/my/key`
- [`s3:DeleteObject`](/terraform/language/backend/s3#s3-deleteobject) on `arn:aws:s3:::mybucket/<workspace_key_prefix>/*/path/to/my/key`
...
- [`dynamodb:DescribeTable`](/terraform/language/backend/s3#dynamodb-describetable)
- [`dynamodb:GetItem`](/terraform/language/backend/s3#dynamodb-getitem)
- [`dynamodb:PutItem`](/terraform/language/backend/s3#dynamodb-putitem)
- [`dynamodb:DeleteItem`](/terraform/language/backend/s3#dynamodb-deleteitem)
...
- [`region`](/terraform/language/backend/s3#region) - (Required) AWS Region of the S3 Bucket and DynamoDB Table (if used). This can also be sourced from the `AWS_DEFAULT_REGION` and `AWS_REGION` environment variables.

The following configuration is optional:

- [`use_lockfile`](/terraform/language/backend/s3#use_lockfile-1) - (Optional) Whether to use a lockfile for locking the state file. Defaults to `false`.
- [`access_key`](/terraform/language/backend/s3#access_key) - (Optional) AWS access key. If configured, must also configure `secret_key`. This can also be sourced from the `AWS_ACCESS_KEY_ID` environment variable, AWS shared credentials file (e.g. `~/.aws/credentials`), or AWS shared configuration file (e.g. `~/.aws/config`).
- [`allowed_account_ids`](/terraform/language/backend/s3#allowed_account_ids) - (Optional) List of allowed AWS account IDs to prevent potential destruction of a live environment. Conflicts with `forbidden_account_ids`.
- [`custom_ca_bundle`](/terraform/language/backend/s3#custom_ca_bundle) - (Optional) File containing custom root and intermediate certificates. Can also be set using the `AWS_CA_BUNDLE` environment variable. Setting ca\_bundle in the shared config file is not supported.
- [`ec2_metadata_service_endpoint`](/terraform/language/backend/s3#ec2_metadata_service_endpoint) - (Optional) Custom endpoint URL for the EC2 Instance Metadata Service (IMDS) API.
  Can also be set with the `AWS_EC2_METADATA_SERVICE_ENDPOINT` environment variable.
- [`ec2_metadata_service_endpoint_mode`](/terraform/language/backend/s3#ec2_metadata_service_endpoint_mode) - (Optional) Mode to use in communicating with the metadata service.
  Valid values are `IPv4` and `IPv6`.
  Can also be set with the `AWS_EC2_METADATA_SERVICE_ENDPOINT_MODE` environment variable.
- [`forbidden_account_ids`](/terraform/language/backend/s3#forbidden_account_ids) - (Optional) List of forbidden AWS account IDs to prevent potential destruction of a live environment. Conflicts with `allowed_account_ids`.
- [`http_proxy`](/terraform/language/backend/s3#http_proxy) - (Optional) URL of a proxy to use for HTTP requests when accessing the AWS API.
  Can also be set using the `HTTP_PROXY` or `http_proxy` environment variables.
- [`https_proxy`](/terraform/language/backend/s3#https_proxy) - (Optional) URL of a proxy to use for HTTPS requests when accessing the AWS API.
  Can also be set using the `HTTPS_PROXY` or `https_proxy` environment variables.
- [`iam_endpoint`](/terraform/language/backend/s3#iam_endpoint) - (Optional, **Deprecated**) Custom endpoint URL for the AWS Identity and Access Management (IAM) API.
  Use `endpoints.iam` instead.
- [`insecure`](/terraform/language/backend/s3#insecure) - (Optional) Whether to explicitly allow the backend to perform "insecure" SSL requests. If omitted, the default value is `false`.
- [`no_proxy`](/terraform/language/backend/s3#no_proxy) - (Optional) Comma-separated list of hosts that should not use HTTP or HTTPS proxies.
...
- [`max_retries`](/terraform/language/backend/s3#max_retries) - (Optional) The maximum number of times an AWS API request is retried on retryable failure. Defaults to 5.
- [`profile`](/terraform/language/backend/s3#profile) - (Optional) Name of AWS profile in AWS shared credentials file (e.g. `~/.aws/credentials`) or AWS shared configuration file (e.g. `~/.aws/config`) to use for credentials and/or configuration. This can also be sourced from the `AWS_PROFILE` environment variable.
- [`retry_mode`](/terraform/language/backend/s3#retry_mode) - (Optional) Specifies how retries are attempted. Valid values are `standard` and `adaptive`. Can also be configured using the `AWS_RETRY_MODE` environment variable or the shared config file parameter `retry_mode`.
- [`secret_key`](/terraform/language/backend/s3#secret_key) - (Optional) AWS access key. If configured, must also configure `access_key`. This can also be sourced from the `AWS_SECRET_ACCESS_KEY` environment variable, AWS shared credentials file (e.g. `~/.aws/credentials`), or AWS shared configuration file (e.g. `~/.aws/config`).
- [`shared_config_files`](/terraform/language/backend/s3#shared_config_files) - (Optional) List of paths to AWS shared configuration files. Defaults to `~/.aws/config`.
- [`shared_credentials_file`](/terraform/language/backend/s3#shared_credentials_file) - (Optional, **Deprecated**, use `shared_credentials_files` instead) Path to the AWS shared credentials file. Defaults to `~/.aws/credentials`.
- [`shared_credentials_files`](/terraform/language/backend/s3#shared_credentials_files) - (Optional) List of paths to AWS shared credentials files. Defaults to `~/.aws/credentials`.
- [`skip_credentials_validation`](/terraform/language/backend/s3#skip_credentials_validation) - (Optional) Skip credentials validation via the STS API.
  Useful for testing and for AWS API implementations that do not have STS available.
- [`skip_region_validation`](/terraform/language/backend/s3#skip_region_validation) - (Optional) Skip validation of provided region name.
- [`skip_requesting_account_id`](/terraform/language/backend/s3#skip_requesting_account_id) - (Optional) Whether to skip requesting the account ID.
  Useful for AWS API implementations that do not have the IAM, STS API, or metadata API.
- [`skip_metadata_api_check`](/terraform/language/backend/s3#skip_metadata_api_check) - (Optional) Skip usage of EC2 Metadata API.
- [`skip_s3_checksum`](/terraform/language/backend/s3#skip_s3_checksum) - (Optional) Do not include checksum when uploading S3 Objects.
  Useful for some S3-Compatible APIs.
- [`sts_endpoint`](/terraform/language/backend/s3#sts_endpoint) - (Optional, **Deprecated**) Custom endpoint URL for the AWS Security Token Service (STS) API.
  Use `endpoints.sts` instead.
- [`sts_region`](/terraform/language/backend/s3#sts_region) - (Optional) AWS region for STS. If unset, AWS will use the same region for STS as other non-STS operations.
- [`token`](/terraform/language/backend/s3#token) - (Optional) Multi-Factor Authentication (MFA) token. This can also be sourced from the `AWS_SESSION_TOKEN` environment variable.
- [`use_dualstack_endpoint`](/terraform/language/backend/s3#use_dualstack_endpoint) - (Optional) Force the backend to resolve endpoints with DualStack capability. Can also be set with the `AWS_USE_DUALSTACK_ENDPOINT` environment variable or in a shared config file (`use_dualstack_endpoint`).
- [`use_fips_endpoint`](/terraform/language/backend/s3#use_fips_endpoint) - (Optional) Force the backend to resolve endpoints with FIPS capability. Can also be set with the `AWS_USE_FIPS_ENDPOINT` environment variable or in a shared config file (`use_fips_endpoint`).

#### Overriding AWS API endpoints

The optional argument `endpoints` contains the following arguments:

- [`dynamodb`](/terraform/language/backend/s3#dynamodb) - (Optional, **Deprecated**) Custom endpoint URL for the AWS DynamoDB API.
...
- [`iam`](/terraform/language/backend/s3#iam) - (Optional) Custom endpoint URL for the AWS IAM API.
...
- [`s3`](/terraform/language/backend/s3#s3-1) - (Optional) Custom endpoint URL for the AWS S3 API.
...
- [`sso`](/terraform/language/backend/s3#sso) - (Optional) Custom endpoint URL for the AWS IAM Identity Center (formerly known as AWS SSO) API.
  This can also be sourced from the environment variable `AWS_ENDPOINT_URL_SSO`.
- [`sts`](/terraform/language/backend/s3#sts) - (Optional) Custom endpoint URL for the AWS STS API.
...
- [`role_arn`](/terraform/language/backend/s3#role_arn) - (Required) Amazon Resource Name (ARN) of the IAM Role to assume.
- [`duration`](/terraform/language/backend/s3#duration) - (Optional) The duration individual credentials will be valid.
...
- [`external_id`](/terraform/language/backend/s3#external_id) - (Optional) External identifier to use when assuming the role.
- [`policy`](/terraform/language/backend/s3#policy) - (Optional) IAM Policy JSON describing further restricting permissions for the IAM Role being assumed.
- [`policy_arns`](/terraform/language/backend/s3#policy_arns) - (Optional) Set of Amazon Resource Names (ARNs) of IAM Policies describing further restricting permissions for the IAM Role being assumed.
- [`session_name`](/terraform/language/backend/s3#session_name) - (Optional) Session name to use when assuming the role.
- [`source_identity`](/terraform/language/backend/s3#source_identity) - (Optional) Source identity specified by the principal assuming the role.
- [`tags`](/terraform/language/backend/s3#tags) - (Optional) Map of assume role session tags.
- [`transitive_tag_keys`](/terraform/language/backend/s3#transitive_tag_keys) - (Optional) Set of assume role session tag keys to pass to any subsequent sessions.
...
- [`role_arn`](/terraform/language/backend/s3#role_arn-1) - (Required) Amazon Resource Name (ARN) of the IAM Role to assume.
  Can also be set with the `AWS_ROLE_ARN` environment variable.
- [`duration`](/terraform/language/backend/s3#duration-1) - (Optional) The duration individual credentials will be valid.
  Credentials are automatically renewed up to the maximum defined by the AWS account.
  Specified using the format `<hours>h<minutes>m<seconds>s` with any unit being optional.
  For example, an hour and a half can be specified as `1h30m` or `90m`.
  Must be between 15 minutes (15m) and 12 hours (12h).
- [`policy`](/terraform/language/backend/s3#policy-1) - (Optional) IAM Policy JSON describing further restricting permissions for the IAM Role being assumed.
- [`policy_arns`](/terraform/language/backend/s3#policy_arns-1) - (Optional) Set of Amazon Resource Names (ARNs) of IAM Policies describing further restricting permissions for the IAM Role being assumed.
- [`session_name`](/terraform/language/backend/s3#session_name-1) - (Optional) Session name to use when assuming the role.
  Can also be set with the `AWS_ROLE_SESSION_NAME` environment variable.
- [`web_identity_token`](/terraform/language/backend/s3#web_identity_token) - (Optional) The value of a web identity token from an OpenID Connect (OIDC) or OAuth provider.
  One of `web_identity_token` or `web_identity_token_file` is required.
- [`web_identity_token_file`](/terraform/language/backend/s3#web_identity_token_file) - (Optional) File containing a web identity token from an OpenID Connect (OIDC) or OAuth provider.
...
- [`bucket`](/terraform/language/backend/s3#bucket) - (Required) Name of the S3 Bucket.
- [`key`](/terraform/language/backend/s3#key) - (Required) Path to the state file inside the S3 Bucket. When using a non-default [workspace](/terraform/language/state/workspaces), the state path will be `<workspace_key_prefix>/<workspace_name>/<key>` (see also the `workspace_key_prefix` configuration).

The following configuration is optional:

- [`acl`](/terraform/language/backend/s3#acl) - (Optional) [Canned ACL](https://docs.aws.amazon.com/AmazonS3/latest/userguide/acl-overview.html#canned-acl) to be applied to the state and lock files.
- [`encrypt`](/terraform/language/backend/s3#encrypt) - (Optional) Enable [server side encryption](https://docs.aws.amazon.com/AmazonS3/latest/userguide/UsingServerSideEncryption.html) of the state and lock files.
- [`endpoint`](/terraform/language/backend/s3#endpoint) - (Optional, **Deprecated**) Custom endpoint URL for the AWS S3 API.
  Use `endpoints.s3` instead.
- [`force_path_style`](/terraform/language/backend/s3#force_path_style) - (Optional, **Deprecated**) Enable path-style S3 URLs (`https://<HOST>/<BUCKET>` instead of `https://<BUCKET>.<HOST>`).
- [`kms_key_id`](/terraform/language/backend/s3#kms_key_id) - (Optional) Amazon Resource Name (ARN) of a Key Management Service (KMS) Key to use for encrypting the state and lock files. Note that if this value is specified, Terraform will need `kms:Encrypt`, `kms:Decrypt` and `kms:GenerateDataKey` permissions on this KMS key.
- [`sse_customer_key`](/terraform/language/backend/s3#sse_customer_key) - (Optional) The key to use for encrypting state and lock files with [Server-Side Encryption with Customer-Provided Keys (SSE-C)](https://docs.aws.amazon.com/AmazonS3/latest/userguide/ServerSideEncryptionCustomerKeys.html). This is the base64-encoded value of the key, which must decode to 256 bits. This can also be sourced from the `AWS_SSE_CUSTOMER_KEY` environment variable, which is recommended due to the sensitivity of the value. Setting it inside a terraform file will cause it to be persisted to disk in `terraform.tfstate`.
- [`use_path_style`](/terraform/language/backend/s3#use_path_style) - (Optional) Enable path-style S3 URLs (`https://<HOST>/<BUCKET>` instead of `https://<BUCKET>.<HOST>`).
- [`workspace_key_prefix`](/terraform/language/backend/s3#workspace_key_prefix) - (Optional) Prefix applied to the state path inside the bucket. This is only relevant when using a non-default workspace. Defaults to `env:`.
...
adjustments to this approach to account for *existing* practices within your
...
separate *administrative* AWS account which contains the user accounts used by
...
- Each role's *Assume Role Policy* must grant access to the administrative AWS
...
[Edit this page on GitHub](https://github.com/hashicorp/web-unified-docs/blob/main/content/terraform/v1.14.x/docs/language/backend/s3.mdx)