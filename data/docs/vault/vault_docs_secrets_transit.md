...
- Vault
- [v1.20.x](/vault/docs/v1.20.x/secrets/transit)
- [v1.19.x](/vault/docs/v1.19.x/secrets/transit)
- [v1.18.x](/vault/docs/v1.18.x/secrets/transit)
- [v1.17.x](/vault/docs/v1.17.x/secrets/transit)
- [v1.16.x](/vault/docs/v1.16.x/secrets/transit)
- ---
...
compromise. For AES-GCM keys, rotation should occur before approximately 232
...
- [`aes128-gcm96`](/vault/docs/secrets/transit#aes128-gcm96): AES-GCM with a 128-bit AES key and a 96-bit nonce; supports
  encryption, decryption, key derivation, and convergent encryption
- [`aes256-gcm96`](/vault/docs/secrets/transit#aes256-gcm96): AES-GCM with a 256-bit AES key and a 96-bit nonce; supports
  encryption, decryption, key derivation, and convergent encryption (default)
- [`chacha20-poly1305`](/vault/docs/secrets/transit#chacha20-poly1305): ChaCha20-Poly1305 with a 256-bit key; supports
  encryption, decryption, key derivation, and convergent encryption
- [`ed25519`](/vault/docs/secrets/transit#ed25519): Ed25519; supports signing, signature verification, and key
  derivation
- [`ecdsa-p256`](/vault/docs/secrets/transit#ecdsa-p256): ECDSA using curve P-256; supports signing and signature
  verification
- [`ecdsa-p384`](/vault/docs/secrets/transit#ecdsa-p384): ECDSA using curve P-384; supports signing and signature
  verification
- [`ecdsa-p521`](/vault/docs/secrets/transit#ecdsa-p521): ECDSA using curve P-521; supports signing and signature
  verification
- [`rsa-2048`](/vault/docs/secrets/transit#rsa-2048): 2048-bit RSA key; supports encryption, decryption, signing, and
  signature verification
- [`rsa-3072`](/vault/docs/secrets/transit#rsa-3072): 3072-bit RSA key; supports encryption, decryption, signing, and
  signature verification
- [`rsa-4096`](/vault/docs/secrets/transit#rsa-4096): 4096-bit RSA key; supports encryption, decryption, signing, and
  signature verification
- [`hmac`](/vault/docs/secrets/transit#hmac): HMAC; supporting HMAC generation and verification.
- [`managed_key`](/vault/docs/secrets/transit#managed_key): Managed key; supports a variety of operations depending on the
  backing key management solution. See [Managed Keys](/vault/docs/enterprise/managed-keys)
  for more information. 

  Enterprise
- [`aes128-cmac`](/vault/docs/secrets/transit#aes128-cmac): CMAC with a 128-bit AES key; supporting CMAC generation and verification. 

  Enterprise
- [`aes192-cmac`](/vault/docs/secrets/transit#aes192-cmac): CMAC with a 192-bit AES key; supporting CMAC generation and verification. 

  Enterprise
- [`aes256-cmac`](/vault/docs/secrets/transit#aes256-cmac): CMAC with a 256-bit AES key; supporting CMAC generation and verification. 

  Enterprise
- [`ml-dsa`](/vault/docs/secrets/transit#ml-dsa) - ML-DSA; supports signing and signature verification (experimental) 

  Enterprise
- [`hybrid`](/vault/docs/secrets/transit#hybrid) - combination of two signature algorithms; supports signing and signature verification (experimental) 

  Enterprise
- [`slh-dsa`](/vault/docs/secrets/transit#slh-dsa) - SLH-DSA (asymmetric) (experimental) 

  Enterprise
- [`aes128-cbc`](/vault/docs/secrets/transit#aes128-cbc) - AES-128 in CBC mode (symmetric, supports derivation and convergent encryption) 

  Enterprise
- [`aes256-cbc`](/vault/docs/secrets/transit#aes256-cbc) - AES-256 in CBC mode (symmetric, supports derivation and convergent encryption) 
...
1. Encrypt some plaintext data using the `/encrypt` endpoint with a named key:
   ...
   **NOTE:** All plaintext data **must be base64-encoded**. The reason for this
   ...
   Note that Vault does not *store* any of this data. The caller is responsible
   ...
   configuration.
2. Decrypt a piece of data using the `/decrypt` endpoint with a named key:
...
3. Rotate the underlying encryption key. This will generate a new encryption key
...
4. Upgrade already-encrypted data to a new key. Vault will decrypt the value
...
- If the HSM supports the CKM\_RSA\_AES\_KEY\_WRAP mechanism, that can be used to wrap the
...
- Otherwise, two mechanisms can be combined to wrap the target key. First, a 256-bit AES key should
  be generated and then used to wrap the target key using the CKM\_AES\_KEY\_WRAP\_KWP mechanism.
  Then the AES key should be wrapped under the wrapping key using the CKM\_RSA\_PKCS\_OAEP mechanism
...
When wrapping an asymmetric key
...
[Edit this page on GitHub](https://github.com/hashicorp/web-unified-docs/blob/main/content/vault/v1.21.x/content/docs/secrets/transit/index.mdx)