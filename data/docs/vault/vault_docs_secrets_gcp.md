# Source: https://developer.hashicorp.com/vault/docs/secrets/gcp

v1.21.x (latest)

- Vault
- [v1.20.x](/vault/docs/v1.20.x/secrets/gcp)
- [v1.19.x](/vault/docs/v1.19.x/secrets/gcp)
- [v1.18.x](/vault/docs/v1.18.x/secrets/gcp)
- [v1.17.x](/vault/docs/v1.17.x/secrets/gcp)
- [v1.16.x](/vault/docs/v1.16.x/secrets/gcp)
- ---
...
[upgrade guide](/vault/docs/secrets/gcp#deprecation-of-access-token-leases)
...
![Partial screenshot of the Vault GUI showing the "Identity token key" search select input](https://web-unified-docs-hashicorp.vercel.app/api/assets/vault/latest/img/gui/gcp/identityTokenKey.png)
...
2. Login under the target namespace or choose the target namespace from the
...
5. Select **Google Cloud**.
6. Click **Next**.
7. Set the mount path for the GCP plugin. For example, `gcp`.
8. If you use WIF, add the identity token key:
...
10. Click **Save** to enable the plugin.
...
![Partial screenshot of the Vault GUI showing the Configuration form](https://web-unified-docs-hashicorp.vercel.app/api/assets/vault/latest/img/gui/gcp/configure.png)

1. Open the web UI for your Vault instance.
2. Login under the target namespace or choose the target namespace from the
   selector at the bottom of the left-hand menu and re-authenticate.
3. Select **Secrets Engines** from the left-hand menu.
4. Select your `gcp` plugin you want to update.
5. Click **Configure**.
...
If you are running Vault from inside [Google Compute Engine](https://cloud.google.com/compute/) or [Google
Kubernetes Engine](https://cloud.google.com/kubernetes-engine/), the instance or pod service account can be used in
place of specifying the credentials JSON file.
For more information on authentication, see the [authentication section](/vault/docs/secrets/gcp#authentication) below.
...
[plugin workload identity federation](/vault/docs/secrets/gcp#plugin-workload-identity-federation-wif)
(WIF) as shown below.
...
![Partial screenshot of the Vault GUI showing the Configuration form with WIF access type selected](https://web-unified-docs-hashicorp.vercel.app/api/assets/vault/latest/img/gui/gcp/configure-wif.png)
...
The Vault UI only supports GCP Secret engine configuration. You must use the CLI/API for additional operations.
...
[things to note](/vault/docs/secrets/gcp#things-to-note) section below.
...
the [bindings](/vault/docs/secrets/gcp#bindings) section below.

For more information on the differences between OAuth2 access tokens and
Service Account keys, see the [things to note](/vault/docs/secrets/gcp#things-to-note) section
below.

For more information on creating and managing rolesets, see the
[GCP secrets engine API docs](/vault/api-docs/secret/gcp) docs.
...
[Google Cloud Service Account](https://cloud.google.com/compute/docs/access/service-accounts). Take note of the email address of the service
...
This endpoint generates a new [GCP IAM service account key](https://cloud.google.com/iam/docs/service-accounts#service_account_keys) associated
...
note](/vault/docs/secrets/gcp#things-to-note) section below.
...
- **Full resource name** - a schema-less URI consisting of a DNS-compatible API
  ...
  documentation](https://cloud.google.com/apis/design/resource_names#full_resource_name) for more information.
...
- **Relative resource name** - A path-noscheme URI path, usually as accepted by
  ...
  documentation](https://cloud.google.com/apis/design/resource_names#relative_resource_name) for more information.
...
roles](https://cloud.google.com/iam/docs/understanding-roles). The roles may be specified in the following formats:
...
Cloud](https://cloud.google.com/docs/authentication/production#providing_credentials_to_your_application). In addition to specifying `credentials` directly via Vault
...
2. The identity of a Google Cloud [workload](https://cloud.google.com/iam/docs/workload-identities). When Vault server is running
   on a Google workload like [Google Compute Engine](https://cloud.google.com/compute/) or [Google Kubernetes Engine](https://cloud.google.com/kubernetes-engine/),
   identity associated with the workload is automatically used. To configure Google Compute
   Engine with an identity, see [attached service accounts](https://cloud.google.com/iam/docs/workload-identities#attached-service-accounts). To
   configure Google Kubernetes Engine with an identity, see [GKE workload identity](https://cloud.google.com/iam/docs/workload-identities#kubernetes-workload-identity).

For more information on service accounts, please see the [Google Cloud Service
Accounts documentation](https://cloud.google.com/compute/docs/access/service-accounts).
...
- Create a [custom role](https://cloud.google.com/iam/docs/creating-custom-roles) using these permissions, and assign this
...
This feature requires [Vault Enterprise](https://www.hashicorp.com/products/vault)(opens in new tab).
...
2. Create a
   ...
   2. Uniquely identify the recipient of the plugin identity token as the audience.
...
3. [Authenticate a workload](https://cloud.google.com/iam/docs/workload-identity-federation-with-other-providers#authenticate)
...
4. Configure the GCP secrets engine with the OIDC audience value and service account
   email.
...
[rotation\_window](/vault/api-docs/secret/gcp#rotation_window) during which the
...
be rotated. See the [GCP API documentation](/vault/api-docs/secret/gcp) for more information on
rotation.
...
- Each GCP project has a limit on the number of IAM Service Accounts. You can
  [request additional quota](https://cloud.google.com/compute/quotas). The quota increase is processed by humans,
...
[vault-plugin-secrets-gcp repo on GitHub](https://github.com/hashicorp/vault-plugin-secrets-gcp).

## API

The GCP secrets engine has a full HTTP API. Please see the [GCP secrets engine API docs](/vault/api-docs/secret/gcp)
for more details.
...
revocation or manage the token TTL - more specifically, **the access\_token
...
- Remove references from `lease_id`, `lease_duration` or other `lease_*`
  ...
  tokens](/vault/docs/secrets/gcp#access-tokens) to see the new format for the response.
...
[Edit this page on GitHub](https://github.com/hashicorp/web-unified-docs/blob/main/content/vault/v1.21.x/content/docs/secrets/gcp.mdx)