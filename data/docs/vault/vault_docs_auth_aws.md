...
- Vault
- [v1.20.x](/vault/docs/v1.20.x/auth/aws)
- [v1.19.x](/vault/docs/v1.19.x/auth/aws)
- [v1.18.x](/vault/docs/v1.18.x/auth/aws)
- [v1.17.x](/vault/docs/v1.17.x/auth/aws)
- [v1.16.x](/vault/docs/v1.16.x/auth/aws)
- ---
...
**Usage:** See the [Authentication](/vault/docs/auth/aws#authentication) section for Vault CLI
and API usage examples. The [Code Example](/vault/docs/auth/aws#code-example) section provides a
...
[![Vault AWS EC2 Authentication Flow](https://web-unified-docs-hashicorp.vercel.app/api/assets/vault/latest/img/vault-aws-ec2-auth-flow.png)](https://web-unified-docs-hashicorp.vercel.app/api/assets/vault/latest/img/vault-aws-ec2-auth-flow.png)

1. An AWS EC2 instance fetches its [AWS Instance Identity Document](http://docs.aws.amazon.com/AWSEC2/latest/UserGuide/instance-identity-documents.html)
   from the [EC2 Metadata Service](http://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-instance-metadata.html). In addition to data itself, AWS
...
3. Vault verifies the signature on the PKCS#7 document, ensuring the information
...
4. Provided all steps are successful, Vault returns the initial Vault token to
...
- What type of entity is authenticated:
  - The ec2 auth method authenticates only AWS EC2 instances and is specialized
    ...
    instances with a specialized tag value (via the role\_tag feature).
...
- [`ec2:DescribeInstances`](/vault/docs/auth/aws#ec2-describeinstances) is necessary when you are using the `ec2` auth method
...
- [`iam:GetInstanceProfile`](/vault/docs/auth/aws#iam-getinstanceprofile) is used when you have a `bound_iam_role_arn` in the
  `ec2` auth method. Vault needs to determine which IAM role is attached to the
  instance profile.
- [`iam:GetUser`](/vault/docs/auth/aws#iam-getuser) and `iam:GetRole` are used when using the iam auth method and
...
- The `sts:AssumeRole` stanza is necessary when you are using [Cross Account
  Access](/vault/docs/auth/aws#cross-account-access). The `Resource`s specified should be a list of
...
[rotation\_window](/vault/api-docs/auth/aws#rotation_window) during which the
...
2. Create an
   ...
   in AWS.
3. The provider URL **must** point at your [Vault plugin identity token issuer](/vault/api-docs/secret/identity/tokens#read-plugin-workload-identity-issuer-s-openid-configuration) with the
   `/.well-known/openid-configuration` suffix removed. For example:
   `https://host:port/v1/identity/oidc/plugins`.
4. Uniquely identify the recipient of the plugin identity token as the audience.
...
5. Create a [web identity role](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_create_for-idp_oidc.html#idp_oidc_Create)
   in AWS with the same audience used for your IAM OIDC identity provider.
6. Configure the AWS auth engine with the IAM OIDC audience value and web
   identity role ARN.
...
When 'role\_tag' option is enabled on a role, the instances are required to have a
...
Policy](/vault/docs/auth/aws#recommended-vault-iam-policy) except it doesn't need any further
...
Anytime Vault uses the [instance metadata service](http://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-instance-metadata.html) on an EC2
...
(IMDSv2)](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/configuring-instance-metadata-service.html). The AWS SDK used by Vault first attempts to connect
...
EC2 instances](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/configuring-instance-metadata-service.html#configuring-instance-metadata-options). With the docker example, setting the hop
...
[plugin workload identity federation](/vault/docs/auth/aws#plugin-workload-identity-federation-wif)
(WIF).
...
[Edit this page on GitHub](https://github.com/hashicorp/web-unified-docs/blob/main/content/vault/v1.21.x/content/docs/auth/aws.mdx)