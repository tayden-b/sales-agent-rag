# Source: https://developer.hashicorp.com/vault/docs/secrets/azure

v1.21.x (latest)

- Vault
- [v1.20.x](/vault/docs/v1.20.x/secrets/azure)
- [v1.19.x](/vault/docs/v1.19.x/secrets/azure)
- [v1.18.x](/vault/docs/v1.18.x/secrets/azure)
- [v1.17.x](/vault/docs/v1.17.x/secrets/azure)
- [v1.16.x](/vault/docs/v1.16.x/secrets/azure)
- ---
...
![Partial screenshot of the Vault GUI showing the "Identity token key" search select input](https://web-unified-docs-hashicorp.vercel.app/api/assets/vault/latest/img/gui/azure/identityTokenKey.png)
...
5. Select **Azure**.
6. Click **Next**.
7. Set the mount path for the Azure plugin. For example, `azure`.
...
![Partial screenshot of the Vault GUI showing the Configuration form](https://web-unified-docs-hashicorp.vercel.app/api/assets/vault/latest/img/gui/azure/configure.png)

1. Open the web UI for your Vault instance.
2. Login under the target namespace or choose the target namespace from the
   selector at the bottom of the left-hand menu and re-authenticate.
3. Select **Secrets Engines** from the left-hand menu.
4. Select your `azure` plugin you want to update.
5. Click **Configure**.
...
If you are running Vault inside an Azure VM with MSI enabled, `client_id` and `client_secret` may be omitted. For more information on authentication, see the [authentication](/vault/docs/secrets/azure#authentication) section below.
...
[plugin workload identity federation](/vault/docs/secrets/azure#plugin-workload-identity-federation-wif)
(WIF) as shown below.
...
![Partial screenshot of the Vault GUI showing the Configuration form with WIF access type selected](https://web-unified-docs-hashicorp.vercel.app/api/assets/vault/latest/img/gui/azure/configure-wif.png)
...
The Vault UI only supports Azure Secret engine configuration. You must use the CLI/API for role configuration and credential rotation.
...
TTL. For more information on roles see the [roles](/vault/docs/secrets/azure#roles) section below.
...
[rotation\_window](/vault/api-docs/secret/azure#rotation_window) during which the
...
2. If only `role_name` is provided, a case-insensitive search-by-name is made, succeeding
...
[configuration](/vault/api-docs/secret/azure#configure-access) section of the API docs.
...
differ depending on if you're using [dynamic or existing](/vault/docs/secrets/azure#choosing-between-dynamic-or-existing-service-principals)
service principals.
...
| --- | --- | --- |
| [User Access Administrator](https://learn.microsoft.com/en-us/azure/role-based-access-control/built-in-roles#user-access-administrator) | Subscription | Service Principal ID given in configuration |
...
2. Configure a
   ...
   2. The subject identifier **must** match the unique `sub` claim issued by plugin identity tokens.
      The subject identifier should have the form `plugin-identity:<NAMESPACE_ID>:secret:<AZURE_MOUNT_ACCESSOR>`.
   3. The audience should be under 600 characters. The default value in Azure is `api://AzureADTokenExchange`.
3. Configure the Azure secrets engine with the subscription, client and tenant IDs and the OIDC audience value.
...
regard to *which* credential was used for an operation.
...
[vault-plugin-secrets-azure repo](https://github.com/hashicorp/vault-plugin-secrets-azure) on GitHub.
...
The Azure secrets engine has a full HTTP API. Please see the [Azure secrets engine API docs](/vault/api-docs/secret/azure)
for more details.

[Edit this page on GitHub](https://github.com/hashicorp/web-unified-docs/blob/main/content/vault/v1.21.x/content/docs/secrets/azure.mdx)