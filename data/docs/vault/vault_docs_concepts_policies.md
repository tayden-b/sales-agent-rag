...
- Vault
- [v1.20.x](/vault/docs/v1.20.x/concepts/policies)
- [v1.19.x](/vault/docs/v1.19.x/concepts/policies)
- [v1.18.x](/vault/docs/v1.18.x/concepts/policies)
- [v1.17.x](/vault/docs/v1.17.x/concepts/policies)
- [v1.16.x](/vault/docs/v1.16.x/concepts/policies)
- ---
...
[![Vault Auth Workflow](https://web-unified-docs-hashicorp.vercel.app/api/assets/vault/latest/img/vault-policy-workflow.svg)](https://web-unified-docs-hashicorp.vercel.app/api/assets/vault/latest/img/vault-policy-workflow.svg)
...
2. The security team authors a policy (or uses an existing policy) which grants
   access to paths in Vault. Policies are written in HCL in your editor of
   preference and saved to disk.
3. The policy's contents are uploaded and stored in Vault and referenced by name.
   You can think of the policy's name as a pointer or symlink to its set of rules.
4. Most importantly, the security team maps data in the auth method to a policy.
...
[![Vault Auth Workflow](https://web-unified-docs-hashicorp.vercel.app/api/assets/vault/latest/img/vault-auth-workflow.svg)](https://web-unified-docs-hashicorp.vercel.app/api/assets/vault/latest/img/vault-auth-workflow.svg)
...
2. Vault establishes a connection to LDAP and asks the LDAP server to verify the
...
3. Vault maps the result from the LDAP server to policies inside Vault using the
...
4. Vault returns the token to the user. This token has the correct policies
...
the authentication steps again, they will get a *new* token. The token will have
...
Policies are written in [HCL](https://github.com/hashicorp/hcl) or JSON and describe which paths in Vault a
...
Both paths end in `*` but the first is shorter. So we end at rule (4), and give `"secret/*"` *lower* priority.

**Informational:**The glob character referred to in this documentation is the asterisk (`*`).
It *is not a regular expression* and is only supported **as the last character of the path**!
...
- [`create`](/vault/docs/concepts/policies#create) (`POST/PUT`) - Allows creating data at the given path. Very few
...
- [`read`](/vault/docs/concepts/policies#read) (`GET`) - Allows reading the data at the given path.
- [`update`](/vault/docs/concepts/policies#update) (`POST/PUT`) - Allows changing the data at the given path. In most
  parts of Vault, this implicitly includes the ability to create the initial
  value at the path.
- [`patch`](/vault/docs/concepts/policies#patch) (`PATCH`) - Allows partial updates to the data at a given path.
- [`delete`](/vault/docs/concepts/policies#delete) (`DELETE`) - Allows deleting the data at the given path.
- [`list`](/vault/docs/concepts/policies#list) (`LIST`) - Allows listing values at the given path. Note that the
...
- [`sudo`](/vault/docs/concepts/policies#sudo) - Allows access to paths that are *root-protected*. Tokens are not
  ...
  privileges.
- [`deny`](/vault/docs/concepts/policies#deny) - Disallows access. This always takes precedence regardless of any
  other defined capabilities, including `sudo`.
- [`subscribe`](/vault/docs/concepts/policies#subscribe) - Allows subscribing to [events](/vault/docs/concepts/events)
  for the given path.
- [`recover`](/vault/docs/concepts/policies#recover) - Allows recovering the data on the given path from a snapshot
...
credentials *creates* database credentials, but the HTTP request is a GET which
...
templates, you will need to get its *mount accessor* and access it via the
`aliases` key.
...
**Note:** The use of [globs](/vault/docs/concepts/policies#policy-syntax) (`*`) may result in [surprising or unexpected behavior](/vault/docs/concepts/policies#parameter-constraints-limitations).
...
- [`required_parameters`](/vault/docs/concepts/policies#required_parameters) - A list of parameters that must be specified.
  ...
  ```
- [`allowed_parameters`](/vault/docs/concepts/policies#allowed_parameters) - A list of keys and values that are
...
- [`denied_parameters`](/vault/docs/concepts/policies#denied_parameters) - A list of keys and values that are not permitted on the given
...
The following operation will error, because "no\_store" is set to false:

```
$ vault write secret/foo no_store=false value=bar
```

Whereas the following operation will succeed, even if the "no\_store"
...
the "no\_store" parameter. All it sees is that the denied parameter isn't present
in the command.

This can be resolved by requiring the "no\_store" parameter in your policy:
...
because there is no "no\_store" parameter:
...
- [`min_wrapping_ttl`](/vault/docs/concepts/policies#min_wrapping_ttl) - The minimum allowed TTL that clients can specify for a
...
- [`max_wrapping_ttl`](/vault/docs/concepts/policies#max_wrapping_ttl) - The maximum allowed TTL that clients can specify for a
  wrapped response.
...
*anything* within Vault. As such, it is **highly recommended** that you revoke
...
However, the *contents* of policies are parsed in real-time whenever the token is used.
...
You cannot make policy adjustments or overwrites to the `ui/mounts`  and `ui/resultant-acl` endpoints once you enable the Vault UI. Vault ignores policy updates that target these paths with [explicit `deny`](/vault/docs/concepts/policies#deny) capabilities.
...
[Edit this page on GitHub](https://github.com/hashicorp/web-unified-docs/blob/main/content/vault/v1.21.x/content/docs/concepts/policies.mdx)