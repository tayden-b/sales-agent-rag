# Source: https://developer.hashicorp.com/vault/docs/auth/azure

v1.21.x (latest)

- Vault
- [v1.20.x](/vault/docs/v1.20.x/auth/azure)
- [v1.19.x](/vault/docs/v1.19.x/auth/azure)
- [v1.18.x](/vault/docs/v1.18.x/auth/azure)
- [v1.17.x](/vault/docs/v1.17.x/auth/azure)
- [v1.16.x](/vault/docs/v1.16.x/auth/azure)
- ---
...
of the following claims to pass validation: "xms\_mirid" or "xms\_az\_rid". These
...
| Resource type | System-assigned management identity | "xms\_mirid" | "xms\_az\_rid" |
| --- | --- | --- | --- |
...
- [`{resource-group-name}`](/vault/docs/auth/azure#resource-group-name) must match `resource_group_name`.
- [`{virtual-machine-name}`](/vault/docs/auth/azure#virtual-machine-name) must match `vm_name` or `{vmss_name}_{instance-id}` if `vmss_name` is provided.
...
| Virtual Machine Scale Set ([Uniform Orchestration](https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/virtual-machine-scale-sets-orchestration-modes#scale-sets-with-uniform-orchestration)) | [`vmss_name`](/vault/api-docs/auth/azure#vmss_name) | `Microsoft.Compute/virtualMachineScaleSets/*/read` |
| Virtual Machine Scale Set ([Flexible Orchestration](https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/virtual-machine-scale-sets-orchestration-modes#scale-sets-with-flexible-orchestration)) | [`vmss_name`](/vault/api-docs/auth/azure#vmss_name) | `Microsoft.Compute/virtualMachineScaleSets/*/read` `Microsoft.ManagedIdentity/userAssignedIdentities/*/read` |
| Services that ([support managed identities](https://learn.microsoft.com/en-us/azure/active-directory/managed-identities-azure-resources/managed-identities-status)) for Azure resources | [`resource_id`](/vault/api-docs/auth/azure#resource_id) | `read` on the resource used to obtain the JWT |
...
2. Configure the Azure auth method:
   ...
   (WIF) as shown below.
3. Alternatively, configure the audience claim value and the Client, Tenant IDs for plugin workload identity federation:
...
[rotation\_window](/vault/api-docs/auth/azure#rotation_window) during which the
...
24hrs and is not configurable. See ([limitations of using managed identities](https://learn.microsoft.com/en-us/azure/active-directory/managed-identities-azure-resources/managed-identity-best-practice-recommendations#limitation-of-using-managed-identities-for-authorization))
for more info.
...
3. Configure the Azure auth method with the client and tenant IDs and the OIDC audience value.
...
([Injector annotations reference](/vault/docs/auth/azure#)).
...
[Edit this page on GitHub](https://github.com/hashicorp/web-unified-docs/blob/main/content/vault/v1.21.x/content/docs/auth/azure.mdx)