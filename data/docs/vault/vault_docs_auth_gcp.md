# Source: https://developer.hashicorp.com/vault/docs/auth/gcp

v1.21.x (latest)

- Vault
- [v1.20.x](/vault/docs/v1.20.x/auth/gcp)
- [v1.19.x](/vault/docs/v1.19.x/auth/gcp)
- [v1.18.x](/vault/docs/v1.18.x/auth/gcp)
- [v1.17.x](/vault/docs/v1.17.x/auth/gcp)
- [v1.16.x](/vault/docs/v1.16.x/auth/gcp)
- ---
...
See [Generating JWTs](/vault/docs/auth/gcp#generating-jwts) for ways to obtain the JWT token.
...
See [API docs](/vault/api-docs/auth/gcp) for expected response.
...
2. Configure the auth method credentials if Vault is not running on Google Cloud:
   ...
   [Google Cloud Credentials](/vault/docs/auth/gcp#gcp-credentials) section below.
   ...
   **Note**: If you're using a [Private Google Access](https://cloud.google.com/vpc/docs/configure-private-google-access)
   ...
   via the [custom\_endpoint](/vault/api-docs/auth/gcp#custom_endpoint) configuration parameter.
   ...
   [plugin workload identity federation](/vault/docs/auth/gcp#plugin-workload-identity-federation-wif)
   (WIF) as shown below.
3. Alternatively, configure the audience claim value and the service account email to assume for plugin workload identity federation:
...
4. Create a named role:
   ...
   [API documentation](/vault/api-docs/auth/gcp).
...
Cloud](https://cloud.google.com/docs/authentication/production#providing_credentials_to_your_application).
...
4. Configure the GCP auth method with the OIDC audience value and service account
   email.
...
[rotation\_window](/vault/api-docs/auth/gcp#rotation_window) during which the
...
that adds [group aliases](/vault/api-docs/secret/identity/group-alias) to the auth response. These
...
[![Vault Google Cloud IAM Login Workflow](https://web-unified-docs-hashicorp.vercel.app/api/assets/vault/latest/img/vault-gcp-iam-auth-workflow.svg)](https://web-unified-docs-hashicorp.vercel.app/api/assets/vault/latest/img/vault-gcp-iam-auth-workflow.svg)

1. The client generates a signed JWT using the Service Account Credentials
   [`projects.serviceAccounts.signJwt`](https://cloud.google.com/iam/docs/reference/credentials/rest/v1/projects.serviceAccounts/signJwt) API method. For examples
   of how to do this, see the [Generating JWTs](/vault/docs/auth/gcp#generating-jwts) section.
...
[![Vault Google Cloud GCE Login Workflow](https://web-unified-docs-hashicorp.vercel.app/api/assets/vault/latest/img/vault-gcp-gce-auth-workflow.svg)](https://web-unified-docs-hashicorp.vercel.app/api/assets/vault/latest/img/vault-gcp-gce-auth-workflow.svg)

1. The client obtains an [instance identity metadata token](https://cloud.google.com/compute/docs/instances/verifying-instance-identity)
   on a GCE instance.
...
This describes how to use the GCP Service Account Credentials [API method](https://cloud.google.com/iam/docs/reference/credentials/rest/v1/projects.serviceAccounts/signJwt)
...
1. Vault can automatically discover the identity token on a GCE/GKE instance. This simplifies
   ...
   $ vault login \
     ...
     role="my-gce-role"
   ```
2. The JWT token can also be obtained from the `"service-accounts/default/identity"` endpoint for a
...
[API docs](/vault/api-docs/auth/gcp) for more details.
...
[Edit this page on GitHub](https://github.com/hashicorp/web-unified-docs/blob/main/content/vault/v1.21.x/content/docs/auth/gcp.mdx)